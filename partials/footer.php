<div class="section footer-section footer-section-04" style="background-image: url(assets/images/bg/footer-bg3.jpg)">
    <div class="container">
        <!-- Footer Widget Wrap Start -->
        <div class="footer-widget-wrap">
            <div class="row">
                <div class="col-lg-3 col-sm-6">
                    <!-- Footer Widget Start -->
                    <div class="footer-widget-about">
                        <a class="footer-logo" href="<?=$base_url?>"><img src="assets/images/retelem-logo-black.svg" alt="Logo" /></a>
                      
                        <div class="footer-social">
                            <ul class="social">
                                <li>
                                    <a href="https://www.facebook.com/retelemdz"><i class="fab fa-facebook-f"></i></a>
                                </li>

                                <li>
                                    <a href="https://www.linkedin.com/company/epe-retelem-spa/"><i class="fab fa-linkedin-in"></i></a>
                                </li>
                                <li>
                                    <a href="https://www.instagram.com/epe__retelem__spa/"><i class="fab fa-instagram"></i></a>
                                </li>
                                <li>
                                    <a href="https://www.youtube.com/@EPERETELEMSPA"><i class="fab fa-youtube"></i></a>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <!-- Footer Widget End -->
                </div>
                <div class="col-lg-3 col-sm-6">
                    <!-- Footer Widget Start -->
                    <div class="footer-widget">
                        <h4 class="footer-widget-title">Lien Utile</h4>

                        <div class="widget-link">
                            <ul class="link">
                                <li><a href="#">Terms & Conditions</a></li>
                                <li><a href="#">A propos</a></li>
                                <li><a href="#">Services</a></li>
                                <li><a href="#">Contact</a></li>
                            </ul>
                        </div>
                    </div>
                    <!-- Footer Widget End -->
                </div>
                <div class="col-lg-3 col-sm-6">
                    <!-- Footer Widget Start -->
                    <div class="footer-widget">
                        <h4 class="footer-widget-title">Nos service</h4>

                        <div class="widget-link">
                            <ul class="link">
                                <li><a href="#services">Telphonie</a></li>
                                <li><a href="#services">Video-surveillance</a></li>
                                <li><a href="#services">Anti-incendie</a></li>
                                <li><a href="#services">Réseaux informatique</a></li>
                                <li><a href="#services">Fibre optique</a></li>
                                <li><a href="#services">Controle d'acccès</a></li>
                                <li><a href="#services">Formation</a></li>
                            </ul>
                        </div>
                    </div>
                    <!-- Footer Widget End -->
                    </div>
                <div class="col-lg-3 col-sm-6">
                    <!-- Footer Widget Start -->
                    <div class="footer-widget">
                        <h4 class="footer-widget-title">Contact</h4>

                        <div class="widget-info">
                            <ul>
                                <li>
                                    <div class="info-icon">
                                        <i class="flaticon-phone-call"></i>
                                    </div>
                                    <div class="info-text">
                                        <span><a href="#">021.96.39.29</a></span>
                                    </div>
                                </li>
                                <li>
                                    <div class="info-icon">
                                        <i class="far fa-envelope-open"></i>
                                    </div>
                                    <div class="info-text">
                                        <span><a href="#">info@retelem-dz.com</a></span>
                                    </div>
                                </li>
                                <li>
                                    <div class="info-icon">
                                        <i class="flaticon-pin"></i>
                                    </div>
                                    <div class="info-text">
                                        <span>54, Rue Omar Benaissa B,E,O ALGER</span>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <!-- Footer Widget End -->
                </div>
            </div>
        </div>
        <!-- Footer Widget Wrap End -->
    </div>

    <!-- Footer Copyright Start -->
    <div class="footer-copyright-area">
        <div class="container">
            <div class="footer-copyright-wrap">
                <div class="row align-items-center">
                    <div class="col-lg-12">
                        <!-- Footer Copyright Text Start -->
                        <div class="copyright-text text-center">
                            <p>© Copyrights <?=date("Y")?> EPE RETELM SPA All rights reserved.</p>
                            <p>By <a href="https://www.instagram.com/developing_off/">developing_off</a></p>
                        </div>
                        <!-- Footer Copyright Text End -->
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- Footer Copyright End -->
</div>